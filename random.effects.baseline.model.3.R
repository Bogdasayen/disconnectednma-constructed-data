# Random effects on baseline models (without single-arm studies)
# Howard Thom 25-September-2017

# Random effects on baseline, binomial likelihood with logistic link. They are modifications of the NICE TSD2 1c (RE) and 1d (FE) functions.
...
model.random.effects.baseline.re<-function()
{ # *** PROGRAM STARTS 
...
model.random.effects.baseline.fe<-function()
{
	for(i in 1:ns){ # LOOP THROUGH STUDIES 
		mu[i] ~ dnorm(m,tau.m) # random effect on baselines
		for (k in 1:na[i]) { # LOOP THROUGH ARMS 
			r[i,k] ~ dbin(p[i,k],n[i,k]) # Binomial likelihood 
