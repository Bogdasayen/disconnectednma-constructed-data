...
model.random.effects.baseline.disc.3cov.re<-function()
{ # *** PROGRAM STARTS 
	# Model for RCTs ##############################################
	...
	for(i in 1:3){
		beta.base[i]~dnorm(0,0.298) #  vague prior for covariate effects
	}
	...
	for(i in 1:ns.disc){ # LOOP THROUGH STUDIES 
		...
		for (k in 1:na.disc[i]) { # LOOP THROUGH ARMS 
			...
			# Fixed intercept
			logit(p.disc[i,k]) <- m.cut + delta.disc[i,k] + x.disc[cov.index[1],i,k]*beta.base.cut[1] #+ x.disc[cov.index[2],i,k]*beta.base.cut[2] #+ x.disc[cov.index[3],i,k]*beta.base.cut[3]  # model for linear predictor 
			# Random intercept
			#logit(p.disc[i,k]) <- mu.disc.cut[i] + delta.disc[i,k] + x.disc[cov.index[1],i,k]*beta.base.cut[1] #+ x.disc[cov.index[2],i,k]*beta.base.cut[2] #+ x.disc[cov.index[3],i,k]*beta.base.cut[3]  # model for linear predictor 
...
model.random.effects.baseline.disc.3cov.fe<-function()
{
	# Model for RCTs ###################################
	for(i in 1:ns){ # LOOP THROUGH STUDIES 
		mu[i] ~ dnorm(0,0.298) # random effect on baselines
		for (k in 1:na[i]) { # LOOP THROUGH ARMS 
			r[i,k] ~ dbin(p[i,k],n[i,k]) # Binomial likelihood 
